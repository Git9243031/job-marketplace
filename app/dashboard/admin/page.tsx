      ...
      <div className="flex gap-1 mb-8 bg-[#F8FAFC] border border-[#E5E7EB] rounded-[12px] p-1">
        {(Object.keys(TAB_LABELS) as Tab[]).map((t) => (
          <button
            ...
            onClick={() => setTab(t)}
            className={cn(
              "flex-1 py-2 text-xs font-medium rounded-[8px] transition-all",
              tab === t
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <StatCard
              ...
              sub={`${stats.pubJobs} опубликовано`}
              color="bg-[#7C3AED]"
            />
            <StatCard
              icon={CheckCircle}
            ...
            <StatCard
              ...
              sub={`${stats.pubResumes} опубликовано`}
              color="bg-[#10B981]"
            />
            <StatCard
              icon={Users}
                ...
                .map((j) => (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-[#64748B]">{j.company}</p>
                    </div>
                    <button
                      onClick={() => toggleJobVis(j.id, j.visible)}
                      className="shrink-0 ml-3 h-7 px-2.5 bg-[#10B981] hover:bg-[#059669] text-white text-xs rounded-[6px] transition-colors"
                    >
                      Одобрить
                    </button>
                  </div>
                ))}
              {jobs.filter((j) => !j.visible).length === 0 && (
            ...
            <div className="bg-white rounded-[20px] border border-[#E5E7EB] p-6">
              ...
              {jobs.slice(0, 5).map((j) => (
                ...
                >
                  ...
                  <span
                    className={cn(
                      "shrink-0 ml-3 text-[10px] font-semibold px-1.5 py-0.5 rounded-full",
                      j.visible
                        ? "bg-green-50 text-green-700"
                        : "bg-amber-50 text-amber-700",
                    )}
                  >
                    {j.visible ? "Опубл." : "Модерация"}
                  </span>
                </div>
              ))}
            </div>
      ...
      {tab === "jobs" && (
        <div className="space-y-3">
          ...
          {jobs.map((j) => (
            <div
              key={j.id}
              className="bg-white rounded-[14px] border border-[#E5E7EB] p-4 flex items-center justify-between gap-4"
            >
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  >
                    {j.visible ? "Опубликовано" : "На модерации"}
                  </span>
                </div>
                <p className="text-xs text-[#64748B]">
                  {j.company} ·{" "}
                  {j.sphere ? SPHERE_RU[j.sphere] || j.sphere : ""} ·{" "}
                  {formatSalaryShort(j.salary_min, j.salary_max) ||
                    "ЗП не указана"}{" "}
              ...
              <div className="flex gap-1.5">
                <button
                  onClick={() => toggleJobVis(j.id, j.visible)}
                  className={cn(
                    "h-8 px-3 text-xs font-medium rounded-[6px] flex items-center gap-1 transition-colors",
                    j.visible
                ...
                <button
                  ...
                  className="w-8 h-8 rounded-[6px] bg-[#FEF2F2] text-[#EF4444] hover:bg-[#FEE2E2] flex items-center justify-center transition-colors"
                >
                  <Trash2 size={12} />
                </button>
              </div>
          ...
          {resumes.map((r) => (
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-[#64748B]">
                  {r.title} · {r.location || "Локация не указана"} ·{" "}
                  {formatDate(r.created_at)}
                </p>
              </div>
              <div className="flex gap-1.5">
                <button
                  onClick={() => toggleResumeVis(r.id, r.visible)}
                  className={cn(
                    "h-8 px-3 text-xs font-medium rounded-[6px] flex items-center gap-1 transition-colors",
                    r.visible
                      ? "bg-[#F1F5F9] text-[#64748B] hover:bg-[#E5E7EB]"
        ...
        <div className="max-w-2xl space-y-4">
          <div className="bg-white rounded-[20px] border border-[#E5E7EB] p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-[#64748B]">
                  Показывать/скрывать приветственный блок с заголовком и
                  статистикой
                </p>
                <p
                  className={cn(
              ...
              <button onClick={toggleHeader}>
                {settings.header_enabled ? (
                  <ToggleRight
                    size={40}
                    className="text-[#7C3AED] cursor-pointer hover:opacity-80 transition-opacity"
                  />
                ) : (
                  <ToggleLeft
                    size={40}
                    className="text-[#94A3B8] cursor-pointer hover:opacity-80 transition-opacity"
                  />
                )}
              </button>
            </div>
          ...
          <div className="bg-white rounded-[20px] border border-[#E5E7EB] p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-[#64748B]">
                  Публиковать вакансии сразу без ручного одобрения
                </p>
                <p
                  className={cn(
                    "text-xs mt-1 font-medium",
                ...
                >
                  {settings.auto_approve_jobs
                    ? "✅ Вкл — вакансии публикуются сразу"
                    : "⛔ Выкл — вакансии ждут модерации"}
                </p>
              </div>
          ...
          <div className="bg-white rounded-[20px] border border-[#E5E7EB] p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-semibold text-[#0F172A] mb-0.5">
                  ✈️ Автопостинг в Telegram
                </h3>
                <p className="text-sm text-[#64748B]">
                  Отправлять новые вакансии в канал при публикации
                </p>
                <p
                  className={cn(
                    "text-xs mt-1 font-medium",
                ...
                >
                  {settings.auto_approve_telegram
                    ? "✅ Вкл — новые вакансии летят в TG"
                    : "⛔ Выкл — только ручная отправка"}
                </p>
              </div>
