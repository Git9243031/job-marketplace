...
function SpherePickerModal({ open, onClose, sphere, subSphere, onChange }: {
  ...
  useEffect(() => { if (open) setActiveSphere(sphere || 'it') }, [open, sphere])
  if (!open) return null
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-[20px] shadow-[0_24px_60px_rgba(0,0,0,0.15)] w-full max-w-[660px] flex flex-col overflow-hidden" style={{maxHeight:'80vh'}}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#F1F5F9]">
          <h2 className="text-base font-semibold text-[#0F172A]">Выбор сферы</h2>
          <button onClick={onClose} className="text-[#94A3B8] hover:text-[#64748B] transition-colors"><X size={18}/></button>
        </div>
        <div className="flex flex-1 overflow-hidden min-h-0">
          <div className="w-48 border-r border-[#F1F5F9] overflow-y-auto py-2 shrink-0">
            {SPHERE_TREE.map(s => (
              <button key={s.value} onClick={() => { setActiveSphere(s.value); onChange(s.value, '') }}
                className={cn('w-full text-left px-5 py-2.5 text-sm transition-colors flex items-center justify-between',
                  activeSphere === s.value ? 'font-semibold text-[#0F172A] bg-[#F8FAFC]' : 'text-[#64748B] hover:text-[#0F172A] hover:bg-[#F8FAFC]')}>
                {s.label}
                {s.children && <ChevronRight size={12} className="text-[#CBD5E1]"/>}
              </button>
            ))}
          </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-[#F1F5F9]">
          <button onClick={() => onChange('', '')} className="text-sm text-[#64748B] hover:text-[#0F172A] transition-colors">Сбросить</button>
...
function SpherePicker({ sphere, subSphere, onClick }: { sphere: string; subSphere: string; onClick: () => void }) {
  const node = SPHERE_TREE.find(s => s.value === sphere)
  const subs = subSphere ? subSphere.split(',').filter(Boolean) : []
  const label = !sphere ? 'Выберите сферу'
    : subs.length === 1 ? `${node?.label} — ${node?.children?.find(c=>c.value===subs[0])?.label ?? subs[0]}`
    : subs.length > 1  ? `${node?.label} (${subs.length} специализации)`
    : node?.label
  return (
    <button type="button" onClick={onClick}
      className={cn('w-full flex items-center justify-between px-4 py-2.5 rounded-[10px] border text-sm transition-all',
        sphere ? 'border-[#DDD6FE] bg-[#F5F3FF] text-[#7C3AED] font-medium'
...
export default function CreateResumePage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    const { error } = await resumesService.createResume({
      ...
      format:        format    || null,
      user_id:       user.id,
      visible:       true,   // сразу публично; поле visible остаётся для будущей модерации
    })
  ...
  return (
    <>
      <SpherePickerModal open={modalOpen} onClose={() => setModalOpen(false)}
        sphere={sphere} subSphere={subSphere}
        onChange={(s, sub) => { setSphere(s); setSubSphere(sub) }}/>
      ...
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        <p className="text-sm text-[#64748B] mb-8">Резюме сразу появится в базе и будет доступно работодателям.</p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div className="bg-white rounded-[16px] border border-[#E5E7EB] p-5 shadow-sm space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-[#374151]">Сфера и специализация</label>
              <SpherePicker sphere={sphere} subSphere={subSphere} onClick={() => setModalOpen(true)}/>
              {subSphere && (
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {subSphere.split(',').filter(Boolean).map(val => {
                    const node = SPHERE_TREE.find(s => s.value === sphere)
                    const child = node?.children?.find(c => c.value === val)
                    return (
                      <span key={val} className="inline-flex items-center gap-1 px-2.5 py-1 bg-[#EDE9FE] text-[#7C3AED] text-xs rounded-full font-medium">
                        {child?.label ?? val}
                        <button type="button" onClick={() => {
                          const next = subSphere.split(',').filter(v => v !== val)
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormInput label="Опыт (лет)" type="number" placeholder="3"
                {...register('experience_years')} error={errors.experience_years?.message}/>
              <FormInput label="Ожидаемая зарплата (₽)" type="number" placeholder="150 000"
          ...
          <div className="bg-white rounded-[16px] border border-[#E5E7EB] p-5 shadow-sm space-y-4">
            <div>
              ...
              <p className="text-xs text-[#94A3B8] mt-0.5">Видны работодателям — заполните хотя бы одно поле</p>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
              ...
              <FormInput label="Email" placeholder="ivan@example.com"
                {...register('email_contact')} error={errors.email_contact?.message}/>
            </div>
          </div>
